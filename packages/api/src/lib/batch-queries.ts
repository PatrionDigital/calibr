...
/**
 * Get multiple counts in a single query using groupBy
 * Instead of: count(where: {a}), count(where: {b}), count(where: {c})
 * Use: groupBy + _count
 */
export async function batchCountByField<T extends string>(
  model: 'position' | 'forecast' | 'userCalibration',
  groupByField: T,
  where?: Record<string, unknown>
): Promise<CountResult> {
  // Type-safe model access
  let groupResult: Array<{ _count: { _all: number } } & Record<T, unknown>>;

  switch (model) {
    case 'position':
      groupResult = await prisma.position.groupBy({
        by: [groupByField as 'platform' | 'outcome'],
        _count: { _all: true },
        where: where as Parameters<typeof prisma.position.groupBy>[0]['where'],
      }) as Array<{ _count: { _all: number } } & Record<T, unknown>>;
      break;
    case 'forecast':
      groupResult = await prisma.forecast.groupBy({
        by: [groupByField as 'isPublic'],
        _count: { _all: true },
        where: where as Parameters<typeof prisma.forecast.groupBy>[0]['where'],
      }) as Array<{ _count: { _all: number } } & Record<T, unknown>>;
      break;
    case 'userCalibration':
      groupResult = await prisma.userCalibration.groupBy({
        by: [groupByField as 'currentTier'],
        _count: { _all: true },
        where: where as Parameters<typeof prisma.userCalibration.groupBy>[0]['where'],
      }) as Array<{ _count: { _all: number } } & Record<T, unknown>>;
      break;
    default:
  ...
  for (const row of groupResult) {
    const fieldValue = String(row[groupByField]);
    const count = row._count._all;
    byField.set(fieldValue, count);
    total += count;
  }

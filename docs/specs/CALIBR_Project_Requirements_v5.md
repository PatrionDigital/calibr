# Calibr.xyz Project Requirements
...
Calibr.xyz is a **prediction market portfolio manager and aggregation layer** that sits on top of existing prediction markets (Polymarket, Kalshi, IEM, etc.). Rather than competing for liquidity by building another prediction market, Calibr provides sophisticated tooling for serious forecasters:
...
const { taskId } = await relay.sponsoredCallERC3009(CCTP_CONTRACT, permit, {
...
```
1. User connects wallet to Calibr.xyz
...
5. User signs Calibr.xyz auth (SIWE) - does NOT expose private key
...
```
1. User connects any EVM wallet to Calibr.xyz
2. Derive their would-be Safe address
3. On first trade: Deploy Safe via Builder Relayer (gasless)
4. Safe is now usable on BOTH Calibr.xyz AND Polymarket.com
...
_Note: Polymarket is now available to US users as of 2024, removing previous geographic restrictions._
...
```mermaid
flowchart TD
    A[User visits Calibr.xyz] --> B{Has Web3 wallet?}
...
// Cache keys
const CATEGORY_CACHE_KEY = (marketId: string) =>
  `market:${marketId}:categories`;
...
_This document represents the complete technical specification for Calibr.xyz's prediction market aggregation platform with EAS-integrated identity infrastructure._  
